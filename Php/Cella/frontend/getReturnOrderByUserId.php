...
include("../php/connect.php");
	...
	$quy = mysql_query("select * from order_product WHERE user_id='$user_id' AND isreturned='1' ");
	if(mysql_num_rows($quy) > 0)
	{
		...
		while($row=mysql_fetch_array($quy)) 
		{
			...
			//customer attribute
			$sql = mysql_query("select * from `order` WHERE user_id='$user_id' AND order_id='$order_id' ");
